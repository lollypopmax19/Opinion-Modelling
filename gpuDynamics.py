...
class GPUDynamics:
    ...
    def __init__(self, network, view):
        ...
        #for i in range(Global.num_nodes):
            #if self.qArray[i] == 1:
                #print("C-Star is: " + str(Global.c_star) + " - real edge_num is: " + str(self.network.get_num_edges(i)))
        ...
        for i in range(Global.num_iterations):
            if self.calcEpsilon() > Global.epsilon:
                ...
                glUniform1f(u_location, self.u)
                self.network.meinungen = _wArray
        ...
        print("-------Result-------")
        print("Cost: " + str(sum(self.uCostCache)))
        print("C_Star: " + str(Global.c_star))
    ...
    @staticmethod
    def count_influenced_nodes(network, c_star):
        """
        Zählt die Anzahl der Knoten, die bei einem bestimmten Wert von C_Star beeinflusst werden.

        :param network: Eine Instanz der Klasse Network.
        :param c_star: Der Wert von C_Star, ab dem ein Knoten als beeinflusst gilt.
        :return: Die Anzahl der beeinflussten Knoten.
        """
        influenced_nodes = 0
        for node in range(Global.num_nodes):
            # Verwendet die Methode get_num_edges der Netzwerk-Instanz
            if network.get_num_edges(node) >= c_star:
                influenced_nodes += 1
